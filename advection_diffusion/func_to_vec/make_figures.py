...
data_prefix = '/media/nnelsen/SharedNHN/documents/datasets/Sandia/raise/results_fnf/'
idx_er_type = 0     # 0 (qoi), 1 (qoi), 2 (loss)
type_list = ['_qoi', '_qoi', '_loss']
yu = [1e0, 1e0, 1e0]
const_list = [1e-2, 1e-2, 8e-3]
n_std = 1

save_plots = not True
plot_folder = "./results/figures_raise_fnf/"
...
for k, data_suffix in enumerate(data_suffix_list):
    ...
    for j, d_str in enumerate(d_str_list):
        ...
        for i, sigma in enumerate(sigma_list):
            ...
            er = er[er.files[0]] 
            er_mean.append(np.mean(er,axis=0)) # QoI, QoI, Loss
...
if save_plots:
    plt.savefig(plot_folder + 'fnf_' + data_type[0] + type_list[idx_er_type] + '.pdf', format='pdf')
...
for k, data_suffix in enumerate(data_suffix_list):
    ...
    er = er[er.files[0]] 
    er_mean = np.mean(er,axis=0) # QoI, QoI, Loss
    std = np.std(er,axis=0)
