...
idx_er_type = 3     # 0 (qoi bochner), 1 (qoi loss), 2 (state bochner), 3 (state loss)
n_std = 2
...
type_list = ['_boch_', '_loss_', '_boch_', '_loss_']
# y_lim_lower = 1e-4
# y_lim_lower = 1e-3
y_lim_lower = 1e-2
yu = [1e0, 1e0, 1e0, 1e0]
# const_list = [1e-2, 1.75e-2, 1.5e-2, 3e-2]
# const_list = [1e-2, 1.15e-1, 1.5e-2, 1.25e-1]
const_list = [1e-2, 1.35e-0, 1.5e-2, 1.5e-0]

if idx_er_type in [0, 1]:
    save_pref = "fno_qoi_"
else:
    save_pref = "fno_state_"
...
model_list = ['FNO2d', 'FNO1d2']
model_str = ['$2d-2d$', '$1d-2d$']
layer_list = ['L2', 'L4']
...
for layer in layer_list:
    for idx_mod, model in enumerate(model_list):
        ...
        for i, N_train in enumerate(N_train_list):
            obj_suffix = '_n' + str(N_train) + '_d' + d_str +"_" + model + "_" + layer + "_" + end_str  + '.npz'
            er = np.load(data_folder + obj_prefix + obj_suffix)
            er = er[er.files[0]] 
            er_mean.append(np.mean(er,axis=0)) # QoIB, QoIL, Bochner, Loss
...
if save_plots:
    plt.savefig(plot_folder + save_pref + data_type[0] + type_list[idx_er_type] + data_suffix[:-7] + '.pdf', format='pdf')
