...
for loop in range(N_MC):
    ...
    model = my_model(d_in, s_outputspace, *modes_width_list, n_layers=n_layers).to(device)
    print(model)
    print("FND parameter count:", count_params(model))
...
if FLAG_save_plots:
    ...
    for i, idx in enumerate(idxs):
        ...
        plt.close()
        fig = plt.figure(figsize=(15 + 7.5, 7.5))
        ...
        ax00 = grid[0].contourf(XX, YY, plot_testsort, vmin=0, vmax=15, cmap=mpl.cm.viridis)
        grid[0].set_title('FND Output', fontsize=16)        
        grid[0].set_xlabel(r'$x_1$')
        grid[0].set_ylabel(r'$x_2$')
        cb00 = grid[0].cax.colorbar(ax00)
        grid[0].cax.toggle_label(True)
        cb00.set_label(r'State')
        ...
        ax11 = grid[2].imshow(er_testsort, origin="lower", interpolation="spline16",\
                       extent=(0,1,0,1), cmap=mpl.cm.viridis)
        grid[2].set_title('Normalized Pointwise Error', fontsize=16)
        grid[2].set_xlabel(r'$x_1$')
        grid[2].set_ylabel(r'$x_2$')
        cb11 = grid[2].cax.colorbar(ax11)
