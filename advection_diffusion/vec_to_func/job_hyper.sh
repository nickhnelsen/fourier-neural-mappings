...
declare -a Ns=("3162")
declare -a models=("FND2d")
declare -a Ls=("2" "4")
declare -a modes=("3" "6" "12" "18")
            ...
            for N in "${Ns[@]}"; do
                for const in "${constants[@]}"; do
                    for m in "${modes[@]}"; do
                        ...
                        std="${dir_name}R-%x.%j"
                        scommand="sbatch --job-name=${job_name} --output=${std}.out --error=${std}.err train_fnd.sbatch ${svpref} ${datsuf} ${N} ${d} ${model} ${L} ${m} ${w} ${m1d} ${w1d}"
