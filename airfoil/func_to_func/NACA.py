...
y_train = output_data[:n_train, :, np.newaxis] 
x_test  = input_data[-n_test:,  :, :] 
y_test  = output_data[-n_test:, :, np.newaxis]
...
for _ in range(5):
    ...
    ################################################################
    model = FNO1d(modes1, width, s_outputspace, d_in=2, d_out=1, n_layers=4, get_grid=False).to(device)
...
# # compute drag and lift
# # in torch, (batch, channels_in, nx_in, ny_in)
# def torch_compute_F_coeff(XYC, p, cnx1 = 50, cnx2 = 120, cny = 50):
#     xx, yy, p = XYC[:,0,cnx1:-cnx1,0], XYC[:,1,cnx1:-cnx1,0], p[:,0,cnx1:-cnx1,0]
#     drag  = torch.einsum('ik,ik->i', (yy[:,0:cnx2]-yy[:,1:cnx2+1]), (p[:,0:cnx2] + p[:,1:cnx2+1])/2.0)
#     lift  = torch.einsum('ik,ik->i', (xx[:,1:cnx2+1]-xx[:,0:cnx2]), (p[:,0:cnx2] + p[:,1:cnx2+1])/2.0)
#     F = torch.column_stack((drag, lift))
...
# y_train = output_data[:n_train, ::r1, ::r2,np.newaxis] 
# x_test  = input_data[-n_test:, ::r1, ::r2, :] 
# y_test  = output_data[-n_test:, ::r1, ::r2, np.newaxis]
...
# # training and evaluation
# ################################################################
# model = FNO2d(modes1, modes2, width, s_outputspace, d_in=2, d_out=1, n_layers=4, get_grid=False).to(device)
