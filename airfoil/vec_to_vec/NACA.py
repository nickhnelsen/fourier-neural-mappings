...
for _ in range(5):
    ...
    # FNF
    modes1 = modes
    model = FNN1d(d_in=7, d_out=2, s_latentspace=64, modes1=modes1, width=width, n_layers=n_layers).to(device)
...
# modes1 = modes
# modes2 = modes
# model = FNN2d(d_in=7, d_out=2, s_latentspace=(64,64), modes1=modes1, modes2=modes2, width=width, n_layers=n_layers).to(device)
...
# model = DNN([7,width,width,width,width,2]).to(device)
