...
class LinearFunctionals2d_diagonal(nn.Module):
    def __init__(self, out_channels, modes1, modes2):
        """
        Fourier neural functionals layer for functions over the 2D torus with DIAGONAL matrix
        Inputs:    
            out_channels (int): total number of linear functionals to extract
        """
        super(LinearFunctionals2d_diagonal, self).__init__()
        ...
        self.weights = nn.Parameter(self.scale * torch.rand(self.out_channels, 2*self.modes1, self.modes2 + 1, dtype=torch.cfloat))
    ...
    def forward(self, x):
        """
        Input shape (of x):     (batch, in_channels, nx_in, ny_in)
        Output shape:           (batch, in_channels)
        """
        # Compute Fourier coeffcients (scaled to approximate integration)
        x = fft.rfft2(x, norm="forward")
        ...
        # Pointwise multiply relevant Fourier modes and take the real part
        x = self.compl_mul2d_pw(x, self.weights).real
...
class FNF2d_lowrank(nn.Module):
    def __init__(self, modes1, modes2, width,
        ...
        d_out           (int): finite number of desired outputs (number of functionals)
        width_lfunc     (int): number of intermediate linear functionals to extract in FNF layer
        rank            (int): rank of matrix-valued function defining the FNF layer
        """
        ...
        self.lfc0 = nn.Linear(self.width, self.rank)
...
class FNF2d_diagonal(nn.Module):
    def __init__(self, modes1, modes2, width,
        ...
        self.lfunc0 = LinearFunctionals2d_diagonal(self.width, self.modes1, self.modes2)
