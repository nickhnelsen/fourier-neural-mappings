...
class LinearFunctional2d(nn.Module):
    ...
    def half_to_full(self, rfft_mat):
        ...
        """
        jmax = rfft_mat.shape[-2]//2
        kmax = rfft_mat.shape[-1] - 1
    ...
    # Complex inner products with the same input function
    def compl_integrate2d(self, input_func, weights):
        # (batch, in_channel, x,y ), (out_channel, x,y) -> (batch, out_channel)
        return torch.einsum("bixy,oxy->bo", input_func, weights)
    ...
    def forward(self, x):
        ...
        x_ft = torch.cat((x_ft[..., :self.modes1 + 1, :self.modes2 + 1], \
                            x_ft[..., -self.modes1:, :self.modes2 + 1]), -2) # truncate modes
        
        # TODO: debug truncation
        assert x_ft.shape == self.weights.shape
        
        x_ft = self.half_to_full(x_ft)
        
        # Extract output of out_channel linear functionals by integrating in Fourier space (\ell^2)
        return torch.real(self.compl_integrate2d(x_ft.conj(), self.half_to_full(self.weights)))
...
class FNO2dfinite(nn.Module):
    def __init__(self, modes1, modes2, width, width_final=128, padding=9, d_out=1, width_func=64):
        ...
        self.width_func = width_func # number of intermediate linear functionals extracted
        ...
        self.fc00 = nn.Linear(self.width, 1) # map down to a single function
        self.func0 = LinearFunctional2d(self.width_func, self.modes1, self.modes2)

        self.fc1 = nn.Linear(self.width_func, self.width_final)
