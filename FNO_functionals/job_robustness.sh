...
do
    for (( i=0; i<=$n_sigma; i++))
    do
        srun --exclusive --ntasks=1 python -u train_fnf.py $save_prefix $data_suffix $N_train $d $i | tee ${save_prefix:0:1}_n${N_train}_d${d}_s${i}_${data_suffix::-7}.out &
    done
    wait
done
echo done
