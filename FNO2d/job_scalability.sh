...
do
    for d in "${ds[@]}"
    do
        srun --exclusive --ntasks=1 python -u train_fno.py $save_prefix $data_suffix $N_train $d $sigma | tee ${save_prefix:0:1}_n${N_train}_d${d}_s${sigma}_${data_suffix::-7}.out &
    done
    wait
    python -u eval_dALL_fno.py $save_prefix $data_suffix $N_train $d_big $sigma
done
echo done
